...
from firewall.models import Alert, Connection, FirewallEvent
...
def validate_features():
    ...
    assert (
        df["connection_count"].dtype == "int32"
        or df["connection_count"].dtype == "int64"
    ), "Incorrect dtype for connection_count"
    ...
    # 4. No duplicate rows
    assert (
        df.duplicated(subset=["src_ip"]).sum() == 0
    ), "Duplicate src_ip groupings detected"
