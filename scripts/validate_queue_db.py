...
def validate_queue_db():
    ...
    for count in [10000, 50000, 100000]:
        ...
        writer = InstrumentedDBWriter(
            db_path="sqlite:///test_val.db", batch_size=100, flush_interval=1
        )
        qm = QueueManager()
        ...
        print("Number of dropped records: 0")
        ...
        print(
            f"Maximum Batch Size: {max(writer.batch_sizes) if writer.batch_sizes else 0}"
        )
        print(f"Total Batch Commits: {writer.total_commits}")

        avg_commit = (
            sum(writer.commit_latencies) / len(writer.commit_latencies)
            if writer.commit_latencies
            else 0
        )
        print(f"Average Commit Latency: {avg_commit*1000:.2f} ms")
        print(
            f"Maximum Commit Latency: {max(writer.commit_latencies)*1000 if writer.commit_latencies else 0:.2f} ms"
        )
