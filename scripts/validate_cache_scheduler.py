...
def validate_cache_scheduler():
    ...
    avg_refresh = (
        sum(refresh_durations) / len(refresh_durations) if refresh_durations else 0
    )
    print(f"Average Cache Refresh Duration: {avg_refresh*1000:.2f}ms")

    print("\n[Cache]")
    # Measure memory
    mem_size = sys.getsizeof(cache.data) + sum(
        sys.getsizeof(v) for v in cache.data.values()
    )
    ...
    print(
        f"Cache Hit Ratio: {counters['hits'] / (counters['hits'] + counters['misses']) * 100:.1f}%"
    )
