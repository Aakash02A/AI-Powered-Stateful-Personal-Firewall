...
def simulate_nmap():
    ...
    for port in range(1, 101):
        p = Packet(timestamp=datetime.now(), src_ip="172.16.0.10", src_port=44444, dst_ip="10.0.0.1", dst_port=port, protocol="TCP", flags="S", size=60)
...
def simulate_hping3():
    ...
    for i in range(100):
        p = Packet(timestamp=datetime.now(), src_ip="172.16.0.20", src_port=30000+i, dst_ip="10.0.0.1", dst_port=80, protocol="TCP", flags="S", size=60)
        fw._process_packet(p)
        
    alerts = fw.database.query_alerts(alert_type="syn_flood")
    if alerts:
        print(f"[+] SUCCESS: Detected Hping3 SYN Flood! Alert: {alerts[0]['description']}")
    else:
...
def simulate_ssh_bruteforce():
    ...
    try:
        ...
        fw = PersonalFirewall(db_path="sqlite:///actual.db")
        for i in range(10):
            p = Packet(timestamp=datetime.now(), src_ip="172.16.0.30", src_port=55555+i, dst_ip="10.0.0.1", dst_port=22, protocol="TCP", flags="S", size=60)
            fw._process_packet(p)
            
        alerts = fw.database.query_alerts(alert_type="brute_force")
        if alerts:
            print(f"[+] SUCCESS: Detected SSH Brute Force! Alert: {alerts[0]['description']}")
        else:
