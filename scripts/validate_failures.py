...
def validate_failures():
    ...
    try:
        writer = DBWriter(
            db_path="sqlite:////invalid/path/db.db", batch_size=10, flush_interval=1
        )
        writer.start()
        ...
        writer.stop()
        print(
            "DBWriter gracefully caught the bad path or flushed cleanly without crashing the main process."
        )
