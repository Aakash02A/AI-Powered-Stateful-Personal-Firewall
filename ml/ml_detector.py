...
class MLAnomalyDetector:
    ...
    def _load_artifacts(self):
        try:
            ...
            if model_path.exists() and scaler_path.exists() and schema_path.exists():
                ...
                self.logger.info(
                    f"Successfully loaded ML anomaly detector from {self.model_dir}"
                )
            else:
                self.logger.warning(
                    f"ML artifacts missing in {self.model_dir}. ML detection disabled."
                )
        ...
        try:
            ...
            for feat_name in self.feature_schema["features"]:
                if not hasattr(conn, feat_name):
                    self.logger.error(
                        f"Missing feature {feat_name} on Connection object."
                    )
                    return False, 0.0
    ...
    def evaluate_batch(self, conns: list[Connection]) -> list[tuple[bool, float]]:
        ...
        try:
            ...
            scores = (
                self.model.decision_function(scaled_features)
                if hasattr(self.model, "decision_function")
                else [0.0] * len(conns)
            )
