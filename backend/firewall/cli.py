...
@cli.command()
@click.option(
    "--config", default="firewall/config/rules.json", help="Path to rules config file"
)
...
def stop():
    ...
    click.echo(
        "To stop the firewall, send SIGINT (Ctrl+C) to the terminal running `start`."
    )
...
def status():
    ...
    click.echo(
        "Please use the dashboard (Tier 2) for live stats or check database for offline logs."
    )
...
def rules(config):
    ...
    try:
        with open(config, "r") as f:
            data = json.load(f)
            click.echo(
                f"{'Rule ID':<25} | {'Action':<8} | {'Protocol':<8} | {'Direction':<10} | {'Description'}"
            )
            click.echo("-" * 80)
            for r in data.get("rules", []):
                click.echo(
                    f"{r.get('rule_id', ''):<25} | {r.get('action', ''):<8} | {r.get('protocol', ''):<8} | {r.get('direction', ''):<10} | {r.get('description', '')}"
                )
...
def alerts(severity):
    ...
    for r in results:
        click.echo(
            f"[{r['timestamp']}] {r['severity'].upper()} - {r['alert_type']}: {r['description']}"
        )
