        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
          {isLoading ? (
            ...
            alerts.map((alert: any) => (
              <div key={alert.id} className="bg-panel rounded-lg p-4 border border-border hover:border-border transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    <div className="mt-1">{getSeverityIcon(alert.severity)}</div>
                    <div>
                      <h4 className="text-foreground font-medium">{alert.alert_type?.replace(/_/g, ' ')}</h4>
                      <p className="text-sm text-muted mt-1">{alert.description}</p>
                      <div className="flex items-center space-x-3 mt-2 text-xs font-mono text-muted">
