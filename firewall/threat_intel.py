...
class ThreatIntelClient:
    def __init__(self, cache_ttl_seconds: int = 86400):
        ...
        if not self.api_key:
            logger.warning(
                "ABUSEIPDB_API_KEY not found in environment. Threat intelligence will run in degraded mode (always returning 0 score)."
            )
    ...
    def _is_local_ip(self, ip_str: str) -> bool:
        try:
            ...
            return (
                True  # If it's an invalid IP, treat it as local to skip external lookup
            )
    ...
    def check_ip(self, ip: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                self.base_url, headers=headers, params=params, timeout=2.0
            )

            if response.status_code == 200:
                data = response.json().get("data", {})
                # Cache the successful response
                self.cache[ip] = {"timestamp": time.time(), "data": data}
                return data
