...
docker-compose up -d
```
The included `docker-compose.yml` ensures that the firewall runs with `--cap-add=NET_ADMIN` and maps the persistent volumes securely.

## 2. Database Migrations (Alembic)

When deploying a new version of the application, you must apply any pending database schema updates.
You can use the CLI tool to automatically upgrade the SQLite database:
```bash
...
Prometheus metrics are automatically exposed on the `/metrics` endpoint. 
To monitor your container, configure your Prometheus instance to scrape `http://<host>:8000/metrics`.
