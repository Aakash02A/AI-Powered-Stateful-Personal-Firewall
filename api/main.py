...
from api.routes import analytics, logs, ws, health
...
async def lifespan(app: FastAPI):
    # Startup
    logger.info(f"Starting Personal Firewall API v1.0.0 on {settings.HOST}:{settings.PORT}", extra={"request_id": "system"})
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    return JSONResponse(
        status_code=422,
        content={"status": "error", "message": "Invalid request parameters", "details": exc.errors()},
    )
