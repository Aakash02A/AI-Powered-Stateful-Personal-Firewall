...
def test_log_packet(db):
    p = Packet(timestamp=datetime.now(), src_ip="1.1.1.1", src_port=123, dst_ip="2.2.2.2", dst_port=456, protocol="TCP", flags="S", size=100)
    db.log_packet(p)
...
def test_log_and_query_alerts(db):
    a = Alert(timestamp=datetime.now(), alert_type="test", severity="low", src_ip="1.1.1.1", dst_ip="2.2.2.2", description="test alert", action_taken="none")
    db.log_alert(a)
