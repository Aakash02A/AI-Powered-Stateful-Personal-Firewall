...
def test_invalid_parameters():
    ...
    response = client.get("/api/v1/alerts?limit=0", headers={"X-API-Key": settings.API_KEY})
...
def test_websocket_auth():
    # Missing API Key
    with pytest.raises(Exception): # websocket.exceptions.ConnectionClosedError or similar depending on test client
        with client.websocket_connect("/api/v1/ws/stream") as websocket:
            pass

def test_websocket_broadcast():
    # Connect with API Key
    with client.websocket_connect(f"/api/v1/ws/stream?api_key={settings.API_KEY}") as websocket:
